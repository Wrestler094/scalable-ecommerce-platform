...
func (r *paymentRepository) Create(ctx context.Context, p domain.Payment) error {
	...
	query := `
        INSERT INTO payments (order_id, user_id, amount, created_at)
        VALUES ($1, $2, $3, $4)
    `

	// Пытаемся получить транзакцию из контекста
	if tx, ok := txmanager.ExtractTx(ctx); ok {
		_, err := tx.ExecContext(ctx, query, daoPayment.OrderID, daoPayment.UserID, daoPayment.Amount, daoPayment.CreatedAt)
		if err != nil {
	...
	_, err := r.db.ExecContext(ctx, query, daoPayment.OrderID, daoPayment.UserID, daoPayment.Amount, daoPayment.CreatedAt)
	if err != nil {
